...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print('Usage: ./insitu_exp.py filename exposure')
    ...
    exp.dev_detector.setup_pilatus(filename=exp.filename, 
                       ...
                       n_frames = 1 # run this script continously in a loop
                        )
    ...
    try:
        ...
        while True:
            ...
            #exp.sprint(f't reset: {time.perf_counter()-t0}') 
            ...
            # for approx. 5 ms
            #exp.sprint(f't before pil: {time.perf_counter()-t0}') 

            # start acquisition
            t1 = time.perf_counter()
            fname = exp.dev_detector.start_pilatus()
            #exp.sprint(f't: {time.perf_counter()-t1}')       
            ...
            # wait untill detector image has been taken
            t1 = time.perf_counter()
            exp.dev_detector.wait_for_detector()
            #exp.sprint(f't: {time.perf_counter()-t1}')       

            exp.sprint(f'Shot taken. Time spent: {time.perf_counter()-t0}')
